...
% [t, Y, pars_out] = SEIRD_model_ThetaSweep(chainst(:,1), pars_phase_1.times, pars_phase_1);
[t, Y, pars_out] = SEIRD_model_ThetaSweep(median(chain_out)', pars_phase_1.times, pars_phase_1);

% diffI =  Calc_dAI_dt(Y, Pars);
% diffA =  Calc_dA_dt(Y, Pars);
diffIsym =  Calc_dI_dt(Y, Pars);
% diffIsym =  Calc_dI_dt(Y, Pars)*chainst(4,1);
...
for j = 1:length(Comps)
    ...
    %     hold on 
    %     plot(ts, diffIsym/Res.mean(4), 's', 'Color', '#0984e3', 'MarkerSize', 10, ...
    % 'MarkerFaceColor','#0984e3', 'DisplayName', 'Serology data')
    %     hold on 
    %     plot(ts, Pars.local_target_mov, 's', 'Color', '#6c5ce7', 'MarkerSize', 10, ...
