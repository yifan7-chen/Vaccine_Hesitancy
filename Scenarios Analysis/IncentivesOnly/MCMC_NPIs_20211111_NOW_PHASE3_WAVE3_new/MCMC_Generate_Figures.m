...
%         saveas(t3, strcat('OUTPUT/', REGION_IN, '/', DATE_IN, '_', REGION_IN, '_',  PARAMETER_SET_IN, '_', LIKELIHOOD_TYPE_IN, "_NVarsFit", int2str(N_VARS_IN), '_', int2str(PLOT_CHAIN_NUM), '_denspanel.png'));

        temp_res = res;
        Chains = cellfun(@(x) x{2}, temp_res, 'un', 0);
        Ress = cellfun(@(x) x{1}, temp_res, 'un', 0);

        t5 = plot_MCMC_results_fig2(500, Chains, ["I","V1","VB"], pars_in, Ress, PHASE_IN);
...
%         t5 = plot_MCMC_results_SupplementarySummaries(100, Chains, ["I"], pars_in, Ress, PHASE_IN, DATE_IN, PARAMETER_SET_IN, LIKELIHOOD_TYPE_IN, N_VARS_IN);
