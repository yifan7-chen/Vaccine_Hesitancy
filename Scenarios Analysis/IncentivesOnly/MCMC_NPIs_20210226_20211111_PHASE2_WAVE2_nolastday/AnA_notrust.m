...
chain_out(:,5) = normrnd(48.90534418,29.82188402,[length(burnOut),1]);

i_samp = randi([1, length(burnOut)], 1,NSamples);
chain_samp = chain_out(i_samp,:);
[t1, Y1, pars_out1] = SEIRD_model_ThetaSweep_notrust(median(chain_out)', pars_phase_2.times, pars_phase_2);
diffIsym1 =  Calc_dI_dt(Y1, Pars);
diffD1=  Calc_dD_dt(Y1, Pars);
diffV1 = Calc_dV1_dt_notrust(Y1,Pars);
...
for j = 1:length(Comps)
    ...
    elseif j_comp == "I"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffIsym1, 's', 'Color', '#0984e3', 'MarkerSize', 10, ...
    'MarkerFaceColor','#0984e3', 'DisplayName', 'Serology data')
        infect_notrust_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.local_target_mov];
    ...
    elseif j_comp == "V1"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffV1, 's', 'Color', '#27ae60', 'MarkerSize', 10, ...
    'MarkerFaceColor','#6c5ce7', 'DisplayName', 'Serology data')
        V1_notrust_CI  = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.vac_count];
