...
% [t, Y, pars_out] = SEIRD_model_ThetaSweep(chainst(:,1), pars_phase_2.times, pars_phase_2);
[t, Y, pars_out] = SEIRD_model_ThetaSweep(median(chain_out)', pars_phase_2.times, pars_phase_2);


% diffI =  Calc_dAI_dt(Y, Pars);
% diffA =  Calc_dA_dt(Y, Pars);
diffIsym =  Calc_dI_dt(Y, Pars);
diffD =  Calc_dD_dt(Y, Pars);
diffV = Calc_V2_dt(Y,Pars);
...
%new add
cumvac =cumsum(vac_count(:,2).Variables);
vac_cumcount  = cumvac(vacidx-1);
...
for j = 1:length(Comps)
    ...
    for i = 1:NSamples
        i_th = chain_samp(i,:);
        sampled_fits(i,:) = SEIRD_model_Theta_cum(i_th, Pars.times, Pars, j_comp, true);
    end
    ...
    initial_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' vac_cumcount];
    % plots
    fill([ts, fliplr(ts)], ...
    ...
    %     hold on 
    %     plot(ts, diffIsym1, 's', 'Color', '#6c5ce7', 'MarkerSize', 10, ...
    ...
    elseif j_comp == "V1"    % Infected
        plot(ts, vac_cumcount, 's', 'Color', '#d63031', 'MarkerSize', 10, ...
         'MarkerFaceColor','#d63031',    'DisplayName', 'Serology data')
        legend({'Reported cases(local symptomatic)'},'Location','northwest', 'fontsize', 18)
        legend boxoff
        yl=ylabel('Infected individuals','fontweight','bold');
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffV, 's', 'Color', '#6c5ce7', 'MarkerSize', 10, ...
    'MarkerFaceColor','#6c5ce7', 'DisplayName', 'initial')
        % V1_cum_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' vac_cumcount];
...
chain_out(:,5) = normrnd(47.90451588,29.38815214,[length(burnOut),1]);

i_samp = randi([1, length(burnOut)], 1,NSamples);
chain_samp = chain_out(i_samp,:);
[t2, Y2, pars_out2] = SEIRD_model_ThetaSweep_notrust(median(chain_out)', pars_phase_2.times, pars_phase_2);
% diffIsym2 =  Calc_dI_dt_new(Y2, Pars);
% diffD2=  Calc_dD_dt_new(Y2, Pars);
diffV2 = Calc_V2_dt(Y2,Pars);
...
for j = 1:length(Comps)
    ...
    for i = 1:NSamples
        i_th = chain_samp(i,:);
        sampled_fits(i,:) = SEIRD_model_Theta_notrust_cum(i_th, Pars.times, Pars, j_comp, true);
    end
    ...
    ci_lower_sampled_fits = mean_sampled_fits - 1.96*std_sampled_fits;
    ci_lower_sampled_fits = max(ci_lower_sampled_fits,0);
    notrust_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' vac_cumcount];
    % plots
    fill([ts, fliplr(ts)], ...
        [ci_higher_sampled_fits, fliplr(ci_lower_sampled_fits)], ...
    ...
    elseif j_comp == "V1"    % Infected
        plot(ts, vac_cumcount, 's', 'Color', '#8e44ad', 'MarkerSize', 10, ...
         'MarkerFaceColor','#d63031',    'DisplayName', 'Serology data')
        legend({'Reported cases(local symptomatic)'},'Location','northwest', 'fontsize', 18)
        legend boxoff
        yl=ylabel('Infected individuals','fontweight','bold');
    ...
    % 'MarkerFaceColor','#0984e3', 'DisplayName', 'Serology data')
        hold on 
        plot(ts, diffV2, 's', 'Color', '#27ae60', 'MarkerSize', 10, ...
    'MarkerFaceColor','#6c5ce7', 'DisplayName', 'no_trust')
...
% [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' vac_cumcount]