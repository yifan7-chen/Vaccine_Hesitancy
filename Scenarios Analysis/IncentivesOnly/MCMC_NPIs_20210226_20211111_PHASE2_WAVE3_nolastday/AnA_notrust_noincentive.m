...
chain_out_notrust = chain_out;

chain_out_notrust(:,6) = normrnd(48.90534418,29.82188402,[length(burnOut),1]);

chain_out_notrust(:,7) = zeros(length(burnOut),1);
chain_out_notrust(:,9) = zeros(length(burnOut),1);
i_samp = randi([1, length(burnOut)], 1,NSamples);
chain_samp_notrust = chain_out_notrust(i_samp,:);
[t_notrust, Y_notrust, pars_out_notrust] = SEIRD_model_ThetaSweep_notrust_initial(median(chain_out_notrust)', pars_phase_2.times, pars_phase_2);
diffIsym_notrust =  Calc_dI_dt_notrust(Y_notrust, Pars);
diffD_notrust =  Calc_dD_dt_notrust(Y_notrust, Pars);
diffV_notrust = Calc_dV1_dt_notrust(Y_notrust,Pars);
...
for j = 1:length(Comps)
    ...
    elseif j_comp == "I"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffIsym_notrust, 's', 'Color', '#0984e3', 'MarkerSize', 10, ...
    ...
    elseif j_comp == "V1"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffV_notrust, 's', 'Color', '#27ae60', 'MarkerSize', 10, ...
