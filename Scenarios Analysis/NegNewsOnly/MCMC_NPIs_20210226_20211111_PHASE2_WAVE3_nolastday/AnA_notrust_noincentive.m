...
%% no incentive
chain_out_noincentive = chain_out;

chain_out_noincentive(:,8) = zeros(length(burnOut),1);
chain_out_noincentive(:,10) = zeros(length(burnOut),1);
i_samp = randi([1, length(burnOut)], 1,NSamples);
chain_samp_noincentive = chain_out_noincentive(i_samp,:);


[t_noincentive, Y_noincentive, pars_out_noincentive] = SEIRD_model_ThetaSweep(median(chain_out_noincentive)', pars_phase_2.times, pars_phase_2);
diffIsym_noincentive =  Calc_dI_dt(Y_noincentive, Pars);
diffD_noincentive=  Calc_dD_dt(Y_noincentive, Pars);
diffV_noincentive = Calc_dV1_dt(Y_noincentive,Pars);
...
for j = 1:length(Comps)
    ...
    'MarkerFaceColor','#6c5ce7', 'DisplayName', 'Serology data')
        V1_noincentive_CI =[mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.vac_count];
    end
    ylim([0, Inf])
