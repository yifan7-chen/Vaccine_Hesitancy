...
% mean_par = mean(chain_out)';
% mean_par(4) = mean_par(4)+chainst(4,2)^2/2;
[t, Y, pars_out] = SEIRD_model_ThetaSweep(median(chain_out)', pars_phase_2.times, pars_phase_2);
% [t, Y, pars_out] = SEIRD_model_ThetaSweep(mean_par, pars_phase_2.times, pars_phase_2);

% diffI =  Calc_dAI_dt(Y, Pars);
% diffA =  Calc_dA_dt(Y, Pars);
diffIsym =  Calc_dI_dt(Y, Pars);
% diffIsym =  Calc_dI_dt(Y, Pars)*chainst(2,1);
