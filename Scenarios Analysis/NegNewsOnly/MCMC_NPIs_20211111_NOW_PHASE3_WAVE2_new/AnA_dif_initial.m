...
[t_noincentive, Y_noincentive, pars_out_noincentive] = SEIRD_model_ThetaSweep_noincentive(median(chain_out)', pars_phase_3.times, pars_phase_3);
diffIsym_noincentive =  Calc_dI_dt_noincentive(Y_noincentive, Pars);
diffD_noincentive=  Calc_dD_dt_noincentive(Y_noincentive, Pars);
diffV_noincentive = Calc_dV1_dt_noincentive(Y_noincentive,Pars);
diffB_noincentive = Calc_dB_dt_noincentive(Y_noincentive,Pars);
...
for j = 1:length(Comps)
    ...
    for i = 1:NSamples
        i_th = chain_samp(i,:);
        sampled_fits(i,:) = SEIRD_model_Theta_noincentive(i_th, Pars.times, Pars, j_comp, true);
    end
    ...
    elseif j_comp == "I"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffIsym_noincentive, 's', 'Color', '#0984e3', 'MarkerSize', 10, ...
    'MarkerFaceColor','#0984e3', 'DisplayName', 'Serology data')
        infect_noincentive_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.local_target_mov];
    elseif j_comp == "R"
    ...
    elseif j_comp == "V1"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffV_noincentive, 's', 'Color', '#27ae60', 'MarkerSize', 10, ...
    'MarkerFaceColor','#6c5ce7', 'DisplayName', 'Serology data')
        V1_noincentive_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.vac_count_P];
    elseif j_comp == "VB"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffB_noincentive, 's', 'Color', '#0984e3', 'MarkerSize', 10, ...
    'MarkerFaceColor','#0984e3', 'DisplayName', 'Serology data')        
        VB_noincentive_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.vac_count_B];
    end
    ylim([0, Inf])
