...
[t_notrustplusincentive, Y_notrustplusincentive, pars_out_notrustplusincentive] = SEIRD_model_ThetaSweep_notrustplusincentive(median(chain_out)', pars_phase_3.times, pars_phase_3);
diffIsym_notrustplusincentive =  Calc_dI_dt_notrustplusincentive(Y_notrustplusincentive, Pars);
diffD_notrustplusincentive=  Calc_dD_dt_notrustplusincentive(Y_notrustplusincentive, Pars);
diffV_notrustplusincentive = Calc_dV1_dt_notrustplusincentive(Y_notrustplusincentive,Pars);
diffB_notrustplusincentive = Calc_dB_dt_notrustplusincentive(Y_notrustplusincentive,Pars);
...
for j = 1:length(Comps)
    ...
    for i = 1:NSamples
        i_th = chain_samp(i,:);
        sampled_fits(i,:) = SEIRD_model_Theta_notrustplusincentive(i_th, Pars.times, Pars, j_comp, true);
    end
    ...
    'MarkerFaceColor','#0984e3', 'DisplayName', 'Serology data')
        V1_notrustplusincentive_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.vac_count_P];
    ...
    elseif j_comp == "VB"    % Infected
        ...
        set(yl,'Pos',[ylabel_xpos pos(2) pos(3)]);
        hold on 
        plot(ts, diffB_notrustplusincentive, 's', 'Color', '#0984e3', 'MarkerSize', 10, ...
    'MarkerFaceColor','#0984e3', 'DisplayName', 'Serology data')       
        VB_notrustplusincentive_CI = [mean_sampled_fits' ci_higher_sampled_fits' ci_lower_sampled_fits' Pars.vac_count_B];
