function loglike = SEIRD_model_LL_logTheta(times, dYdt_target, ThetaLog, Pars, PLOT_RES)
    ...
    dYdt_deceased_target_daily = dYdt_target(:,2);  
    ...
    %% Deceased
    deceased_daily = Calc_dD_dt(y, pars_in);

    xd = dYdt_deceased_target_daily;
    lambdad = deceased_daily;
    lambdad = max(lambdad,0);
    b_nonzerod = find(lambdad~=0);  
    lambdad = lambdad(b_nonzerod);
    xd = xd(b_nonzerod);
    ...
    RESCALE_FACTOR_infect = 1/Pars.N*100; 
    infect_factor = 10;
    RESCALE_FACTOR_death = RESCALE_FACTOR_infect*100;
    ...
    loglike = sum(logpoispdf(xs*RESCALE_FACTOR_infect*infect_factor, lambds*RESCALE_FACTOR_infect*infect_factor))+...
        sum(logpoispdf(xvac*RESCALE_FACTOR_vac*1, lambdvac*RESCALE_FACTOR_vac*1))+...
        sum(logpoispdf(xB*RESCALE_FACTOR_vac*1, lambdB*RESCALE_FACTOR_vac*1))+...
        sum(logpoispdf(xd*RESCALE_FACTOR_death, lambdad*RESCALE_FACTOR_death));
