function V1_new = Calc_dV1_dt_all(Y, Pars,handle)   
    ...
    if handle ==25
        V1_new =[1.05501385405660e-09; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    elseif handle == 50
        V1_new =[6.54836185276508e-11; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    elseif handle == 75
        V1_new =[5.09317032992840e-10; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    else
        V1_new =[6.47560227662325e-10; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    end    
    V1_new = abs(V1_new);
end
