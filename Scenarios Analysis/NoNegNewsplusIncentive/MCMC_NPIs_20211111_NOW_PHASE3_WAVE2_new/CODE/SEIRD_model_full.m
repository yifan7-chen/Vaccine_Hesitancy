function dYdt = SEIRD_model_full(t, X, Pars)
  ...
  elseif t >= Pars.t_feb22_start && t < Pars.t_mar22_start
    ...
    if t<20
        dYdt(Pars.Y2B_ids)          = Pars.kappa_2B * (1 - ratio_2B) .* ratio_2B.*(temp_Iv2_CM - temp_IB_CM + Pars.r_2B.*(Dv2ratio - DBratio) + Pars.threshold_2B+64.6303037 * Pars.im); %+ Pars.npi_payoff
    else
        ...
        dYdt(Pars.Y2vac_ids) = Pars.kappa_2vac * (1 - ratio_2vac) .* ratio_2vac.*(temp_I_CM - temp_Iv2_CM + Pars.r_2vac.*(Dratio - Dv2ratio) + 180.9160127+Pars.threshold_2vac); %+ Pars.npi_payoff
