function dYdt = SEIRD_model_full(t, X, Pars)
    ...
    % elseif t>Pars.center1 && t<=Pars.center2
        % Pars.AllContacts = Pars.AllContacts * Pars.p_sp;
    % % elseif t>Pars.center2 && t<=Pars.center3
    % %     Pars.AllContacts = Pars.AllContacts * Pars.p_sp1;
    else
    ...
    % CM = Pars.AllContacts .* (1 + X(Pars.X_ids).*Pars.contact_reduced(floor(t)+Pars.beforeidx) - Pars.q_fixbasic);
    ...
    % beta1CM = Pars.beta(1) * CM * infec2tot;
    % beta2CM = Pars.beta(2) * CM * infec2tot;
    % beta3CM = Pars.beta(3) * CM * infec2tot;
    % beta4CM = Pars.beta(4) * CM * infec2tot;
    % beta5CM = Pars.beta(5) * CM * infec2tot;
  ...
  elseif t >= Pars.t_feb22_start && t < Pars.t_mar22_start
    ...
    % if t<52
    dYdt(Pars.Y2B_ids)          = Pars.kappa_2B * (1 - ratio_2B) .* ratio_2B.*(temp_Iv2_CM - temp_IB_CM + Pars.r_2B.*(Dv2ratio - DBratio) + Pars.threshold_2B+(64.6303037-67.51979488)*Pars.im); %+ Pars.npi_payoff
    dYdt(Pars.Y2vac_ids) = Pars.kappa_2vac * (1 - ratio_2vac) .* ratio_2vac.*(temp_I_CM - temp_Iv2_CM + Pars.r_2vac.*(Dratio - Dv2ratio) +46.58693725+ Pars.threshold_2vac); %+ Pars.npi_payoff
    
    % elseif t>=52 && t<92
    %     dYdt(Pars.Y2B_ids)          = Pars.kappa_2B * (1 - ratio_2B) .* ratio_2B.*(temp_Iv2_CM - temp_IB_CM + Pars.r_2B.*(Dv2ratio - DBratio) + Pars.threshold1_2B); %+ Pars.npi_payoff
    %     dYdt(Pars.Y2vac_ids) = Pars.kappa_2vac * (1 - ratio_2vac) .* ratio_2vac.*(temp_I_CM - temp_Iv2_CM + Pars.r_2vac.*(Dratio - Dv2ratio) + Pars.threshold1_2vac); %+ Pars.npi_payoff
    % else
    %     dYdt(Pars.Y2B_ids)          = Pars.kappa_2B * (1 - ratio_2B) .* ratio_2B.*(temp_Iv2_CM - temp_IB_CM + Pars.r_2B.*(Dv2ratio - DBratio) + Pars.threshold2_2B); %+ Pars.npi_payoff
    %     dYdt(Pars.Y2vac_ids) = Pars.kappa_2vac * (1 - ratio_2vac) .* ratio_2vac.*(temp_I_CM - temp_Iv2_CM + Pars.r_2vac.*(Dratio - Dv2ratio) + Pars.threshold2_2vac); %+ Pars.npi_payoff
    % 
    % end

    
    % up_delta_2B       = 0.99999999-X(Pars.Y2B_ids);
    % low_delta_2B      = 0.00002-X(Pars.Y2B_ids);
    % dYdt(Pars.Y2B_ids)    = max(min(delta_2B,up_delta_2B),low_delta_2B);
