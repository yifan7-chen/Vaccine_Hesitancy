function V1_new = Calc_dV1_dt_all(Y, Pars,handle)   
    ...
    if handle ==25
        V1_new =[2.58296495303512e-10; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    elseif handle == 50
        V1_new =[6.91215973347426e-10; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    elseif handle == 75
        V1_new =[3.20142135024071e-10; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    else
        V1_new =[6.76664058119059e-10; sum(dYdt_model_V1,2) + sum(dYdt_model_V2,2) + sum(dYdt_model_VB,2)];
    end    
    V1_new = abs(V1_new);
end
